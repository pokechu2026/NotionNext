...
const getIndentLevel = type => {
  if (Number.isFinite(indentLevels[type])) {
    return indentLevels[type]
  }

  const numberedHeader = type.match(/^header_(\d+)$/)
  return numberedHeader ? Math.max(0, Number(numberedHeader[1]) - 1) : 0
}
      ...
      if (type.indexOf('header') >= 0) {
        ...
        if (!existed) {
          toc.push({
            ...
            indentLevel: getIndentLevel(type)
          })
        }

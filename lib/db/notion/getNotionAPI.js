...
// Notion 的 unofficial API 會以 User-Agent 封鎖 Node/undici 的預設值（回傳 403 Forbidden），
// 因此必須帶一個正常的瀏覽器 User-Agent，否則首頁抓不到 Notion 資料，
// 導致網站標題變 undefined、自訂選單退回預設值。可用環境變數覆蓋。
...
function getRawNotion() {
  if (!globalStore.notion) {
    globalStore.notion = new NotionLibrary({
      ...
      // 注意：此版 notion-client(7.7.1) 底層改用 ofetch，建構參數為 ofetchOptions（舊的 kyOptions 會被忽略）。
      ofetchOptions: {
        headers: {

...
const { proxyNotionImage, proxyBlockImages } = require('./notionImgProxy')
const { fetchNotionWithRetry, mapWithConcurrency } = require('./notionFetch')
...
async function queryCourseDB() {
  ...
  try {
    ...
    while (hasMore) {
      ...
      const res = await fetchNotionWithRetry(url, {
        method: 'POST',
        headers,
    ...
    // 解析每筆課程，並抓取縮圖（限制併發數，避免一次發太多請求觸發速率限制）
    const courses = await mapWithConcurrency(allResults, 4, page => parseCourse(page, headers))
    return courses
  } catch (err) {
...
async function parseCourse(page, headers) {
  ...
  // 將 AWS 簽名 URL 轉為代理 URL（防止過期）+ 縮圖壓縮
...
async function fetchCoursePageContent() {
  ...
  try {
    ...
    if (imageBlock) {
      const rawUrl = imageBlock.image?.file?.url ||
        imageBlock.image?.external?.url || null
      bannerUrl = proxyNotionImage(rawUrl, imageBlock.id, 'banner')
    }

    // 講師簡介：synced_block 及其後面所有 block（排除 child_database 和 image）
    const syncedIndex = blocks.findIndex(b => b.type === 'synced_block')
    const blocksAfterSynced = syncedIndex === -1 ? [] : blocks.slice(syncedIndex)
    const lecturerBlocks = await mapWithConcurrency(blocksAfterSynced, 4, block => enrichBlock(block, headers))

    // 將講師區塊中的圖片 URL 轉為代理 URL
    proxyBlockImages(lecturerBlocks)
...
async function enrichBlock(block, headers) {
  ...
  if (block.has_children || block.type === 'synced_block') {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json()
        result.children = await mapWithConcurrency(
          data.results || [], 4, child => enrichBlock(child, headers)
        )
      }
    } catch {
...
async function fetchCoursePage(pageId) {
  ...
  try {
    // 1. 取得頁面 metadata
    const pageRes = await fetchNotionWithRetry(`https://api.notion.com/v1/pages/${pageId}`, { headers })
    if (!pageRes.ok) {
...
async function fetchAllBlocks(blockId, headers) {
  ...
  while (hasMore) {
    ...
    const res = await fetchNotionWithRetry(url.toString(), { headers })
    if (!res.ok) break
  ...
  // 平行取得有子內容的 block（含 synced_block 特殊處理），限制併發數避免觸發速率限制
  await mapWithConcurrency(allBlocks, 4, async block => {
    if (block.has_children || block.type === 'synced_block') {

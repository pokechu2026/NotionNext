...
export function formatNotionBlock(block) {
  ...
  for (let i = 0; i < blocksToProcess.length; i++) {
    ...
    // 【新格式兼容】Notion API 新格式：block.value = { value: {...}, role: 'reader' }
    // react-notion-x 期待 block.value.id 直接存在，需解包內層
    if (b?.value?.value !== undefined && b?.value?.role !== undefined) {
      clonedBlock[blockId] = { ...b, value: b.value.value }
  ...
  // 最終防呆：react-notion-x 會對每個 block 的 value.id 呼叫 uuidToId(id.replaceAll)，
  // 只要有一個 block 的 value.id 缺失，就會丟出未捕捉例外，導致「整篇文章內文空白 / 卡在載入」。
  // Notion 非官方 API 常見兩種問題資料：
  //   1) 權限不足的區塊：value = { role: 'none' }（無 id、無內容，根本渲染不出來）→ 直接丟棄
  //   2) value 為空/非物件的殘缺區塊 → 直接丟棄
  //   3) 其餘僅缺 id 的，用 map 的 key（本身即該 block 的 UUID）回填
  for (const key of Object.keys(clonedBlock)) {
    const entry = clonedBlock[key]
    if (!entry || !entry.value || typeof entry.value !== 'object') {
      delete clonedBlock[key]
      continue
    }
    // 權限不足 / 無法解析的區塊（只有 role，沒有真正的 block 內容）
    if (entry.value.role === 'none' || entry.value.type === undefined) {
      delete clonedBlock[key]
      continue
    }
    if (!entry.value.id) {
...
/**
 * 補抓 blockMap 中缺少的子 block（解決 Notion API 新格式下 missing block 問題）
 * notion-client 因為無法識別新格式的 block.type，不會遞迴抓取子 block，
 * 導致 value.content 裡的 ID 在 blockMap 找不到。
 * 此函數掃描所有 content 陣列，補抓缺少的 block，最多遞迴 5 層。
 * @param {Object} blockMap - 已 formatNotionBlock 過的 blockMap
 * @returns {Promise<Object>} - 補齊後的 blockMap
 */
export async function fetchMissingBlocks(blockMap) {
  ...
  for (let i = 0; i < MAX_ITERATIONS; i++) {
    ...
    Object.values(blockMap).forEach(block => {
      ...
      content.forEach(id => {
        if (!blockMap[id]) missingIds.push(id)
      })
    })

    if (missingIds.length === 0) break
    console.log(`[fetchMissingBlocks] round ${i + 1}: 補抓 ${missingIds.length} 個 missing block`)

    const fetched = await fetchInBatches([...new Set(missingIds)])
    const normalized = formatNotionBlock(fetched)
    Object.assign(blockMap, normalized)
  }
  return blockMap
}

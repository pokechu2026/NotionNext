...
async function queryPortfolioDB(filter) {
  ...
  try {
    ...
    // 限制併發數，避免一次發太多請求觸發速率限制
    return mapWithConcurrency(allResults, 4, page => parseProject(page, headers))
  } catch (err) {
...
async function parseProject(page, headers) {
  ...
  // 將 S3 簽名 URL 轉為穩定代理 URL（若 fetchFirstImage 已處理則保持不變）
...
async function fetchProjectPage(pageId) {
  ...
  try {
    ...
    const project = await parseProject(page, headers)

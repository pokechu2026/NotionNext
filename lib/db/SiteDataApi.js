...
import { fetchInBatches, fetchNotionPageBlocks, formatNotionBlock, fetchMissingBlocks } from '@/lib/db/notion/getPostBlocks'
...
const { proxyNotionImage } = require('./notion/notionImgProxy')
...
export async function getSiteDataByPageId({ pageId, from }) {
  // Conversion also fetches missing blocks. Cache the finished result so every
  // generated route does not repeat the same expensive Notion work.
  return getOrSetDataWithCache(
    `site_data_converted_${pageId}`,
    async (pageId, from) => {
...
/**
 * 用 Official Notion API 抓頁面第一個 image block 的 URL，作為縮圖 fallback
 * 結果會快取，避免重複呼叫 API
 */
async function fetchFirstImageForPost(pageId, token) {
  return getOrSetDataWithCache(`post_thumb_${pageId}`, async () => {
    try {
      ...
      for (const block of data.results || []) {
        if (block.type === 'image') {
          ...
          if (!url) continue
          return proxyNotionImage(url, block.id, 'thumbnail')
        }
      }
      return null
    } catch (err) {
      console.warn('[Blog] Failed to fetch first image for post', pageId, err.message)
      return null
    }
  })
}
...
async function getNotice(post) {
  ...
  if (rawBlockMap?.block) {
    post.blockMap = { ...rawBlockMap, block: formatNotionBlock(rawBlockMap.block) }
  } else {
    post.blockMap = rawBlockMap
  }
  return post
}
...
if (post?.id && !post?.blockMap) {
    try {
      ...
      const normalizedBlock = formatNotionBlock(post.blockMap.block)
      const completeBlock = await fetchMissingBlocks(normalizedBlock)
      post.blockMap = { ...post.blockMap, block: completeBlock }
...
async function convertNotionToSiteData(SITE_DATABASE_PAGE_ID, from, pageRecordMap) {
  ...
  for (let i = 0; i < pageIds.length; i++) {
    ...
    try {
      ...
      console.warn('⚠️ 解析页面属性失败，跳过此页面:', id, error.message)
      continue
    }
  ...
  // 補齊缺失縮圖：對沒有 pageCoverThumbnail 的 Post 用 Official API 抓第一張圖
  // 每批 3 個循序處理，避免同時打太多請求觸發 Notion rate limit (3 req/s)
  const notionToken = process.env.NOTION_ACCESS_TOKEN
  if (notionToken) {
    const postsWithoutThumb = collectionData.filter(p => p.type === 'Post' && !p.pageCoverThumbnail)
    const BATCH = 3
    for (let i = 0; i < postsWithoutThumb.length; i += BATCH) {
      await Promise.all(
        postsWithoutThumb.slice(i, i + BATCH).map(async post => {
          const cover = await fetchFirstImageForPost(post.id, notionToken)
          if (cover) post.pageCoverThumbnail = cover
        })
      )
    }
  }
...
function handleDataBeforeReturn(db) {
  // Notion 暫時失敗時仍要回傳可序列化的穩定資料形狀，讓舊快取頁面
  // 或降級畫面可以繼續服務，而不是因 undefined 讓整次建置中止。
  ...
  db.categoryOptions = Array.isArray(db.categoryOptions) ? db.categoryOptions : []
...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  ...
  const defaultDescription =
    NOTION_CONFIG?.DESCRIPTION || BLOG.DESCRIPTION || '这是一个由NotionNext生成的站点'
  ...
  const pageCover = defaultPageCover !== '/bg_image.jpg'
    ? defaultPageCover
    : collection?.cover
